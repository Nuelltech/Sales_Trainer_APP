...
// --- MAIN COMPONENT ---
export default function AppAutomovel() {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          mediaRecorder.onstop = async () => {
            ...
            try {
              // Deepgram API Call
              const res = await fetch(`/deepgram/v1/listen?language=pt-PT&model=nova-3&smart_format=true&keyterm=WLTP&keyterm=Wallbox&keyterm=kW&keyterm=kWh&keyterm=supercharger&keyterm=autonomia&keyterm=h%C3%ADbrido%20plug-in`, {
                method: 'POST',
                body: audioBlob
              });
  ...
  const getFeedback = async () => {
    ...
    const prompt = `Analisa esta conversa de treino de vendas e dá feedback como um Coach de Vendas Automóvel especializado em Veículos Elétricos.\n\nPERFIL DO CLIENTE: ${profile.name} — ${profile.role}. ${profile.personality}
    ...
    return (
      <div style={styles.root}>
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>EV Training</h1>
          <p style={styles.subtitle}>Treina a venda de automóveis elétricos</p>
        </div>
