...
// --- MAIN COMPONENT ---
export default function AppFarmaceutica() {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          mediaRecorder.onstop = async () => {
            ...
            try {
              // Deepgram API Call
              const res = await fetch(`/deepgram/v1/listen?language=pt-PT&model=nova-3&smart_format=true&keyterm=off-label&keyterm=farmacocin%C3%A9tica&keyterm=posologia&keyterm=ensaio%20cl%C3%ADnico&keyterm=placebo&keyterm=comparticipa%C3%A7%C3%A3o`, {
                method: 'POST',
                body: audioBlob
              });
  ...
  const getFeedback = async () => {
    ...
    const prompt = `Analisa esta conversa de treino de vendas e dá feedback como um Medical Sales Coach experiente.\n\nPERFIL DO CLIENTE: ${profile.name} — ${profile.role}. ${profile.personality}
    ...
    return (
      <div style={styles.root}>
        <div style={styles.header}>
          <div style={styles.logo}>MED</div>
          <h1 style={styles.title}>Pharma Training</h1>
          <p style={styles.subtitle}>Treina a tua abordagem médica</p>
        </div>
