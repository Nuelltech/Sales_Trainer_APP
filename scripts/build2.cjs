...
apps.forEach(app => {
  ...
  newContent = replaceBetween(newContent, 'const systemPrompt = `', '`;\\n    const messages =', app.systemPrompt);

  // Replace Feedback Criteria
  newContent = replaceBetween(newContent, 'const objectiveCriteria = {', '    };\\n    const prompt = `Analisa', app.objectiveCriteria);
